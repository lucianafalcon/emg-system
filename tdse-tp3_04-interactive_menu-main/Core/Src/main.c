...
int main(void)
{
  ...
  /* ADC1 Init manual por registros - CH0 (PA0) y CH1 (PA1) */
  __HAL_RCC_ADC1_CLK_ENABLE();
  __HAL_RCC_GPIOA_CLK_ENABLE();

  /* PA0 y PA1 como analog input (MODE=00, CNF=00) */
  GPIOA->CRL &= ~(GPIO_CRL_MODE0 | GPIO_CRL_CNF0 |
                  GPIO_CRL_MODE1 | GPIO_CRL_CNF1);

  /* ADC config: scan mode, continuous, 2 conversiones */
  ...
  /* Sampling time 239.5 cycles para CH0 y CH1 */
  ADC1->SMPR2 = (7) | (7 << 3);

  /* Encender ADC y calibrar */
  ADC1->CR2 |= ADC_CR2_ADON;
  HAL_Delay(1);
  ADC1->CR2 |= ADC_CR2_ADON;     /* segundo ADON arranca conversion */
